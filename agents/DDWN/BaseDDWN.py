...
from .ddwn_agent import DDWNA


class BaseDDWN(AbstractAgent):
    """
    Base class for DDWN
    This implementation implements action selection by `a = argmax(sum(softmax(q_value[i]) * w_values[i]))`
    This also updates every single agent's Q and W networks unlike DWN where only non selected agents W-networks are updated
    """
