...
deep_sea_treasure_labels = ["water", "food"]
...
for y, row in enumerate(states):
    for x, value in enumerate(row):
        ...
        if True:
            ...
            current_ax.set_title(f"{weights[max_weight_idx]:.2f}-{np.argmax(q_valuess[max_weight_idx])}", ha="center", va="top", fontsize=4, pad=1)
