from gymnasium.envs.registration import register
from .mountain_car import MOMountainCar


register(id="mo-mountaincar-v0", entry_point="envs.mountain_car.mountain_car:MOMountainCar", max_episode_steps=200)

register(
    id="mo-mountaincar-3d-v0",
    entry_point="envs.mountain_car.mountain_car:MOMountainCar",
    max_episode_steps=200,
    kwargs={"add_speed_objective": True, "merge_move_penalty": True},
)

register(
    id="mo-mountaincar-timemove-v0",
    entry_point="envs.mountain_car.mountain_car:MOMountainCar",
    max_episode_steps=200,
    kwargs={"merge_move_penalty": True},
)

register(
    id="mo-mountaincar-timespeed-v0",
    entry_point="envs.mountain_car.mountain_car:MOMountainCar",
    max_episode_steps=200,
    kwargs={"remove_move_penalty": True, "add_speed_objective": True},
)
